...
def read_electolum_file(file, pack=True, crop=True):
    ...
    for numPattern in range(numPatterns):
        ...
        if crop : y = icrop_image_basic(y)
        list_images.append(y)
        list_images_size.append(y.shape)
        
    list_images_size.sort(key=lambda t: t[1]) # sort list_image_size by ascending width
    width_min = list_images_size[0][1]        # picks the minimum width
    list_images = [img[0:img.shape[0], 0:width_min] for img in list_images] # resize all images
    ...
    return data_struct(
        imgWidth, imgHeight, numPatterns, exptime, info1, info2, info3, np.array(list_images,dtype=object)
    )
